...
export default function ArticlePage({
  ...
  const publishDate = useMemo(() => {
    return currentArticle?._sys?.createdAt
      ? formatDate(currentArticle._sys.createdAt, 'yyyy年M月d日')
      : ''
  }, [currentArticle?._sys?.createdAt])

  const updateDate = useMemo(() => {
    return currentArticle?._sys?.updatedAt
      ? formatDate(currentArticle._sys.updatedAt, 'yyyy年M月d日')
      : ''
  }, [currentArticle?._sys?.updatedAt])
    ...
    <Layout app={app}>
      ...
      <main className={mystyles.containerArticle}>
        ...
        <article v-if="currentArticle">
          ...
          <div className={mystyles.sideBySide}>

          <div className={mystyles.dateItems}>
              <time dateTime={formatDate(publishDate, 'yyyy年M月d日')} className={mystyles.dateItemArticle}>
                {publishDate}
              </time>
              <time dateTime={formatDate(publishDate, 'yyyy年M月d日')} className={mystyles.dateItemArticle}>
              （更新：{updateDate}）
            ...
            {/* <div className={mystyles.dateItems}>
              <time dateTime={publishDate} className={mystyles.dateItemArticle}>
                {publishDate}
              </time>
              <time dateTime={updateDate} className={mystyles.dateItemArticle}>
              （更新：{updateDate}）
              </time> */}
            ...
            <ul>
              {currentArticle?.tags?.map((tag) => (
                <li key={tag._id}>
                  <Link href={`/tag/${tag.slug}`}  className={mystyles.tagItemWithLink}>
                    {/* <a className={mystyles.tagItemWithLink}> */}
                      #{tag.name}
                    {/* </a> */}
                  </Link>
                </li>
          ...
          <h1 className={mystyles.articleTitle}>{currentArticle?.title}</h1>
          <div className={mystyles.articleCover}>
            <Image
              src={currentArticle?.coverImage?.src}
              alt={currentArticle?.coverImage?.altText}
              width={310}
              height={163}
              sizes="100vw"
              style={{
          ...
          <div className={mystyles.articleShare}>
                <p className={mystyles.articleShareLabel}>この記事をシェアする</p>
                <ul className={mystyles.articleShareList}>
                  <li>
                    <a type="button" onClick={shareOnTwitter}>
                      <svg
                  ...
                  <li>
                    <a type="button" onClick={shareOnFacebook}>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        height="24px"
                        viewBox="0 0 24 24"
...
{/*               
          ...
          {/* <ul className={mystyles.tocList}></ul>
          <Script src="/js/TableOfContents.js" /> */}

          <div
            className={mystyles.articleBody}
            dangerouslySetInnerHTML={body}
          ></div>

          <div className={`${mystyles.articleShare} ${mystyles.justifyCenter}`}>
            <p className={mystyles.articleShareLabel}>この記事をシェアする</p>
            <ul className={mystyles.articleShareList}>
              <li>
                <a type="button" onClick={shareOnTwitter}>
          ...
          <aside className={mystyles.articleAuthor}>
            <h2 className={mystyles.articleAuthorTitle}>この記事を書いた人</h2>
            <div className={mystyles.articleAuthorInner}>
              <div className={mystyles.articleAuthorImage}>
                <img
                    src="/images/author/author01-sm.webp"
                    className={mystyles.radiusRound}
                    alt="著者" width="120" height="120"
                  />
              </div>
              <div className={mystyles.articleAuthorProfile}>
                <div className={mystyles.articleAuthorName}>著者名です。</div>
                <div className={mystyles.articleAuthorDescription}>著者の説明です。</div>
              </div>
            </div>
          </aside>
          ...
          <nav className={mystyles.links}>
            {prevArticle && (
              <Link href={`/article/${prevArticle.slug}`} className={mystyles.linksPrevious}>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    height="24px"
                    viewBox="0 0 24 24"
            ...
            {nextArticle && (
              <Link href={`/article/${nextArticle.slug}`} className={mystyles.linksNext}>
                  次の投稿へ
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
...
// function outputTableOfContents(documentRef){var documentRef=documentRef||document;var toc=documentRef.getElementsByClassName("tocList")[0];var headings=[].slice.call(documentRef.body.querySelectorAll('.postBody h2,.postBody h3,.postBody h4,.postBody h5,.postBody h6'));headings.forEach(function(heading,index){var ref="toc"+index;if(heading.hasAttribute("id"))ref = heading.getAttribute("id");else heading.setAttribute("id",ref);var link=documentRef.createElement("a");link.setAttribute("href","#"+ref);link.textContent=heading.textContent;var div=documentRef.createElement("li");div.setAttribute("class",heading.tagName.toLowerCase());div.appendChild(link);toc.appendChild(div);});}try{module.exports=outputTableOfContents;}catch(e){}outputTableOfContents();